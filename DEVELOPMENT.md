...
## 現在の開発方針（重要 / 2026-09-02 に変更）

**主開発対象は Web アプリ版**。リポジトリ直下の `index.html` と `assets/` が本体で、
GitHub Pages でそのまま公開している（<https://naoki213.github.io/atoikura/>）。

### なぜ iOS ネイティブから Web へ切り替えたか

iOS版（`Atoikura/`, `TaxEngine/`）は完成してCIも通っている（後述）。
しかし開発体制として以下の問題があった。

- 依頼者が Mac を持っておらず、購入予定もない。iOSアプリのビルド・実行には Xcode（macOS専用）が必須で、
  **依頼者自身がアプリを触って確認することが一度もできない**状態だった。
- 開発エージェント側の環境にも Xcode が無く、GitHub Actions の macOS ランナー経由でしか
  検証できない。1往復に5〜8分かかり、実行コストも高い（macOSランナーは課金係数が高い）。
- 「動作を見ながら開発したい」という要望に対して、Web版なら
  依頼者はスマホのブラウザで即座に触れ、エージェント側も手元の Chromium（Playwright）で
  操作・撮影して確認できる。

Web版に移しても、**税計算ロジックの設計・境界値テストの内容はそのまま移植できた**ため、
これまでの検討内容は無駄になっていない。

### Web版の設計方針

- **ビルド工程を作らない**。フレームワーク・バンドラ・トランスパイラを使わず、素のHTML/CSS/JSで書く。
  リポジトリのファイルがそのまま公開されるので、「push すれば反映される」状態を保てる。
- 各ファイルは IIFE で `globalThis.Atoikura.*` に登録する。
  こうすると ES modules の CORS 制約を受けず、Node.js からは `require()` するだけで
  同じコードをテストできる（`tools/run-tests.cjs`）。
- **金額は円単位の整数**、税率は basis point の整数で扱う。小数を経由しないので誤差が出ない。
- **日付は 'YYYY-MM-DD' の文字列**で保持し、年月の判定は文字列の前方一致で行う。
  `Date` とタイムゾーンを介さないため、iOS版で実際に踏んだ「タイムゾーンで集計月がずれる」
  類の不具合が原理的に発生しない。
- **文字入力中は再描画しない**（入力欄が作り直されてカーソルが飛ぶのを防ぐ）。
  金額のプレビューや連動する数字は `data-live` を付けた要素だけを部分更新する（`app.updateLiveValues`）。
- 操作は `data-action` 属性で宣言し、`document` レベルのイベント委譲でまとめて受ける。
  再描画のたびにリスナーを貼り直さなくてよい。

### ブラウザで実際に動かして見つけた問題（記録）

コードを書いただけでは気づけず、`tools/screenshots.cjs` で実際に操作して初めて分かったもの。

1. **初回起動時に大きなマイナス金額が出ていた**。データを何も登録していない状態では
   売上0円に対して予備資金と国民年金だけが差し引かれ、ホームに `-¥815,040` と赤字表示されていた。
   初めて開いた人を驚かせる表示なので、売上の情報が何も無いときは金額ではなく
   「売上を登録するか、予測タブで年間の見込みを入力すると計算できます」という案内を出すようにした
   （`home.js` の `hasBasisForForecast`）。
2. **履歴の行で、品目と日付が改行されず繋がっていた**（「売上9月2日」）。
   `span` が inline のままだったため。`.entry-main` を `flex-direction: column` にして修正。

この2件は「実際に動かして目で見る」以外の方法では見つけられなかった。
以後も、画面に関わる変更をしたら `npm run check` を実行してスクリーンショットを確認すること。

## iOS版（参考・現在は主開発対象ではない）

以下は iOS 版の開発時の記録。iOS版のコードとCI（`.github/workflows/ios.yml`）は残してあり、
iOS関連ファイルを変更したときだけ macOS ランナーで検証される。

**iOS版の最終状態: CI（ビルド + 全テスト）がすべて成功している。**
